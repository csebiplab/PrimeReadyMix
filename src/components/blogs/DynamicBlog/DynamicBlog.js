...
const DynamicBlogComponent = ({ params }) => {
    ...
    const parsedContent = useMemo(() => {
        if (categories?.content) {
            ...
            return parser.parseFromString(categories.content, 'text/html');
        }
        return null;
    }, [categories]);
    ...
    useEffect(() => {
        if (!isLoading && parsedContent) {
            ...
            // const strippedHTML = blogDetails.replace(/background-color\s*:\s*(#[a-fA-F0-9]{6}|rgb\s*\(\s*255\s*,\s*255\s*,\s*255\s*\))\s*;?/g, '');
